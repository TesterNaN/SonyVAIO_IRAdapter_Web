...
用一根**支持数据传输的 USB-C OTG 线**把适配器接到安卓手机上，打开本页（https）就能直接用，
**不需要装任何驱动**。原因：安卓走的是内核 usbfs + 系统 USB 授权（Chrome 通过 UsbManager 直接
claim 厂商类接口），没有 Windows 那种"设备必须被 WinUSB 接管"的要求。（实测有效。）

也正因为如此，页面在窄屏上有专门处理：遥控器面板提到最前、画布给一个最小宽度并横向滚动，
让键保持手指大小。
...
| 平台 | 要求 |
|---|---|
| Windows | 装原厂驱动包绑 WinUSB（自制未签名 INF 会被 x64 拒） |
| 安卓 | **零安装**：OTG 线接上、打开 https 页面即可（实测） |
| Linux | 不需要内核驱动，但需要 udev 规则给权限 |
| macOS | 不需要驱动、不需要规则 |
| Firefox / Safari | 不支持 WebUSB |

详细逆向记录（协议逐字节、全部实测结论、踩过的坑）见 `REPORT.md`（如果用 `--with-report` 打包了）。
