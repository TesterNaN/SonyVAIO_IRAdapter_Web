...
## 29. ★ 双机对拍方案（手机零安装 + PC，各接一个红外头）

### 29.1 为什么比以前的对拍强

以前（§12.7 / §13）是在**同一台机器**上对拍：两个适配器都插在 PC 上，靠 Python 工具 A 发 B 收。
要求两个头都绑好 WinUSB、都插在这一台机器上、还得在一个脚本里协调。

现在两个头在**两台不同主机**上：PC 一个、安卓手机一个（§28 零安装），
各自被各自的浏览器独占、互不干扰；两边都能看页面日志、都能一键复制捕获结果。
（以后再插更多头也不冲突 —— 每个头配一台机器/一个浏览器标签即可。）

### 29.2 网页端新增：⑤ 接收 / 学习面板（VER `2026-09-24m`）

| 按钮 | 发的帧 | 期望回应 |
|---|---|---|
| ① 进入学习模式 | `... 00 03 0C 48 90` | `01 01`（两个参数字不被校验，§14.3） |
| ② 扫一遍槽位 | 槽位 0–15 逐个 `... 00 03 12 00 <槽位>` | 有码：`[len] 00 01 <码数据>`（len = 2 + 码长）；空槽：`01 06` |
| ③ 一直等（收到就停） | 反复轮询上面那组槽位 | 直到某个槽位吐出数据，随即停止 |
| 捕获列表 + 复制 | — | 每条显示 槽位 / 字节数 / 十六进制，可一键复制 |

**★ 读一个槽位就会把它清空**（官方应用就是这么取码的，§12.2），所以拿到了就要存下来。

实现上有个细节值得记：轮询空槽位可能回 `01 06` 也可能**干脆不回**，所以
①槽位命令用 `quiet=false`（不触发"超时强制静默 0.8 秒"，否则扫 16 个槽位会拖成二三十秒）；
②把 `01 06` 也算"有效回应"再自己分类，避免它落进"迟到回应"分支被刷屏。
离线自测台 `_harness_cmd.js` 里专门加了 8 项断言（有码/空槽分类、长度校验、扫描不被拖慢等），全过。

### 29.3 标准对拍流程

```
接收端（手机或第二台电脑）：连接设备 → ① 进入学习模式 → ③ 一直等
发射端（PC）：选中遥控器 → 按某个键（要重复就猛点，命令已串行化不会互相冲掉）
接收端：出现捕获 → 「复制捕获结果」→ 贴回来
```

### 29.4 这套方案能立刻回答的三个悬案

1. **学习码的存储格式**：同一个按键重复对拍多次，看字节序列是否稳定、
   是否与 §15"发射时长由装进去的码块长度决定"吻合 —— 这是"红外信息怎么编码"的最后一块。
2. **回放闭环**：把捕获写回（`10 <槽位2B> 90 01 <数据>`）再发射，和原始发射对比。
   §12.3 里我们写回去被回 `01 0A`（拒绝），那是全项目**唯一没闭环的点**；
   双机对拍可以反复复现，验证"必须写回刚读出的那个槽位"之类的猜测。
